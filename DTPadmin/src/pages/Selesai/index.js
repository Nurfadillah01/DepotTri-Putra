import { View, Text, Dimensions, FlatList, TouchableOpacity, Modal, Linking, StatusBar, Alert } from 'react-native'
...
export default function Dikirim() {
    ...
    useEffect(() => {
        const unsubscribe = database().ref('Pesanan/Selesai/').on('value', snap => {
            if (snap.val() == null) {
                setData(null)
            } else {
                let dataArr = []
    ...
    const visibleHandle = (item) => {
        database().ref(`Pesanan/Selesai/${item.key}/Pesanan/`).once('value', snap => {
            if (snap == null) {
                null
            } else {
                let dataArr = []
    ...
    const hapusHandle = () => {
        Alert.alert('', 'Hapus riwayat?', [
            ...
            {
                text: 'Hapus', onPress: () => {
                    database().ref(`User/Account/${id}/History/${keyPesanan}/`).remove()
                    database().ref(`Pesanan/Selesai/${keyPesanan}/`).remove()
                        .then(() => invisibleHandle())
                }
            },
        ])
    }
    ...
    return (
        <View style={{ width, height, backgroundColor: '#EEFCFF' }}>
            ...
            {data == null
                ...
                : <View style={{ height: height - 110, width, padding: 10 }}>
                    <FlatList
                        data={data}
                        renderItem={({ item, index }) => {
                            return (
