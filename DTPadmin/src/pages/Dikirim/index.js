...
export default function Dikirim() {
    ...
    useEffect(() => {
        const unsubscribe = database().ref('Pesanan/Dikirim/').on('value', snap => {
            if (snap.val() == null) {
                setData(null)
            } else {
                let dataArr = []
    ...
    const visibleHandle = (item) => {
        database().ref(`Pesanan/Dikirim/${item.key}/Pesanan/`).once('value', snap => {
            if (snap == null) {
                null
            } else {
                let dataArr = []
    ...
    const selesaiHandle = () => {
        Alert.alert('', 'Selesaikan pesanan?', [
            {
                text: 'Batal'
            },
            {
                text: 'Ya', onPress: () => {
                    const newReference = database().ref(`Pesanan/Selesai/`).push();
                    newReference.set({
                        Alamat: {
                            latitude, longitude, alamat
                        },
                        Info_User: {
                            nama, nomor, jam, tanggal, id: key
                        },
                    })
                    database().ref(`User/Account/${key}/History/${newReference.key}/`).set({
                        Alamat: {
                            latitude, longitude, alamat
                        },
                        Info_User: {
                    ...
                    dataModal.forEach(child => {
                        database().ref(`Pesanan/Selesai/${newReference.key}/Pesanan/${child.key}/`).update({
                            harga: child.harga, nama_barang: child.produk, total_harga: child.total_harga, quantity: child.quantity
                        })
                        database().ref(`Admin/Produk/${child.key}/`).once('value', snap => {
                            const {stock} = snap.val()
                            const quantity = stock - child.quantity
                            database().ref(`Admin/Produk/${child.key}/`).update({
                                stock: quantity
                            })
                        })
                        database().ref(`User/Account/${key}/History/${newReference.key}/Pesanan/${child.key}/`).update({
                            harga: child.harga, nama_barang: child.produk, total_harga: child.total_harga, quantity: child.quantity
                        })
                    })
                    database().ref(`Pesanan/Dikirim/${key}/`).remove().then(() => invisibleHandle())
                }
            },
        ])
    }
