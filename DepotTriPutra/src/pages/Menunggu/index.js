...
export default function Menunggu() {
    ...
    useEffect(() => {
        const unsubscribe = database().ref(`Pesanan/Masuk/${uid}/`).on('value', snap => {
            if (snap.val() == null) {
                removeData()
            } else {
    ...
    useEffect(() => {
        const unsubscribe = database().ref(`Pesanan/Masuk/${uid}/Pesanan/`).on('value', snap => {
            if (snap.val() == null) {
                removeData()
            } else {
                let dataArr = []
    ...
    const batalHandle = () => {
        Alert.alert('', 'Batalkan pesanan?', [
            {
                text: 'Tidak'
            },
            {
                text: 'Ya', onPress: () => {
                    database().ref(`Pesanan/Masuk/${uid}/`).remove()
                }
            },
        ]);
    }
            ...
            {data == ''
                ...
                : <View style={{ flex: 1 }}>
                    ...
                    <View style={{ height: 80, justifyContent: 'space-between' }}>
                        ...
                        <TouchableOpacity
                            onPress={() => batalHandle()}
                            style={{ height: 40, backgroundColor: '#FE4964', justifyContent: 'center', alignItems: 'center', borderRadius: 5 }}
                        >
                            <Text style={{ fontWeight: 'bold', color: '#ffffff' }}>Batalkan Pesanan</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            }
