import { View, Text, Dimensions, FlatList, TouchableOpacity, Modal, Linking, Alert } from 'react-native'
...
export default function Dikirim() {
    ...
    const [keyPesanan, setkeyPesanan] = useState(null)
    ...
    useEffect(() => {
        const unsubscribe = database().ref(`User/Account/${uid}/History/`).on('value', snap => {
            if (snap.val() == null) {
                setData(null)
            } else {
                let dataArr = []
    ...
    const visibleHandle = (item) => {
        database().ref(`User/Account/${uid}/History/${item.key}/Pesanan/`).once('value', snap => {
            if (snap == null) {
                null
            } else {
                let dataArr = []
    ...
    const renderModal = () => {
        ...
        return (
            <View style={{ width, height, backgroundColor: '#f0f0f0' }}>
                <View style={{ height: 60, backgroundColor: '#f0f0f0', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <TouchableOpacity
                ...
                <View style={{ height: height - 60, width, justifyContent: 'space-between', paddingHorizontal: 20 }}>
                    <View style={{}}>
                        ...
                        <View style={{ height: 60, marginBottom: 5 }}>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ fontWeight: 'bold', fontSize: 16, color: '#000000', marginLeft: 20 }}>{keyPesanan}</Text>
                            </View>
                        </View>
                        ...
                        renderItem={({ item, index }) => {
                            return (
                                <TouchableOpacity
                                    ...
                                    key={item.key}
                                    style={{ height: 90, backgroundColor: '#f0f0f0', elevation: 3, borderRadius: 10, margin: 5, flexDirection: 'row', padding: 10, justifyContent: 'space-between' }}>
                                    <View style={{ width: width - 50, justifyContent: 'space-between' }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                            <Text style={{ fontSize: 18, color: '#000000' }}>{item.key}</Text>
